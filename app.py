...
if page == "Engagement Dashboard":
    if filtered.empty:
        ...
        # Create subplots: one box per state
        from plotly.subplots import make_subplots
        states = cat_df['State'].tolist()
        num_states = len(states)
        fig_grp = make_subplots(
        ...
        # Populate each subplot with the four categories
        for idx, row in cat_df.iterrows():
            for cat in ['H1', 'H2', 'Other', 'Untouched']:
                fig_grp.add_trace(
                    go.Bar(
                        x=[cat],
                        y=[row[cat]],
                        marker=dict(color=category_colors.get(cat)),
                        name=cat,
                        showlegend=(idx == 0)  # show legend only once
                    ),
                    row=1, col=idx + 1
                )
            # Hide individual x-axis titles to keep layout clean
            fig_grp.update_xaxes(title_text='', row=1, col=idx + 1)

        # Set the y-axis title on the first subplot only
        fig_grp.update_yaxes(title_text='Number of LHNs', row=1, col=1)

        # Set overall figure layout
        fig_grp.update_layout(
            barmode='group',
            title='LHNs by Category per State (Counts)',
            showlegend=True,
